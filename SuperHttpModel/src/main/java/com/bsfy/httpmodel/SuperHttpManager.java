...
import com.bsfy.httpmodel.cache.model.CacheMode;
import com.bsfy.httpmodel.cookie.CookieManger;
import com.bsfy.httpmodel.https.HttpsUtils;
import com.bsfy.httpmodel.interceptor.HttpLoggingInterceptor;
import com.bsfy.httpmodel.model.HttpHeaders;
import com.bsfy.httpmodel.model.HttpParams;
import com.bsfy.httpmodel.request.CustomRequest;
import com.bsfy.httpmodel.request.DeleteRequest;
import com.bsfy.httpmodel.request.DownloadRequest;
import com.bsfy.httpmodel.request.GetRequest;
import com.bsfy.httpmodel.request.PostRequest;
import com.bsfy.httpmodel.request.PutRequest;
import com.bsfy.httpmodel.utils.HttpLog;
import com.bsfy.httpmodel.utils.RxUtil;
...
public final class SuperHttpManager {
    ...
    private volatile static SuperHttpManager singleton = null;

    private SuperHttpManager() {
    ...
    public static SuperHttpManager getInstance() {
        testInitialize();
        if (singleton == null) {
            synchronized (SuperHttpManager.class) {
                if (singleton == null) {
                    singleton = new SuperHttpManager();
                }
            }
        }
        return singleton;
     ...
     * 调试模式,默认打开所有的异常调试
     */
    public SuperHttpManager debug(String tag) {
        debug(tag, true);
        return this;
    }
     ...
     */
    public SuperHttpManager debug(String tag, boolean isPrintException) {
     ...
     * https的全局访问规则
     */
    public SuperHttpManager setHostnameVerifier(HostnameVerifier hostnameVerifier) {
     ...
     * https的全局自签名证书
     */
    public SuperHttpManager setCertificates(InputStream... certificates) {
     ...
     * https双向认证证书
     */
    public SuperHttpManager setCertificates(InputStream bksFile, String password, InputStream... certificates) {
     ...
     * 全局cookie存取规则
     */
    public SuperHttpManager setCookieStore(CookieManger cookieManager) {
     ...
     * 全局读取超时时间
     */
    public SuperHttpManager setReadTimeOut(long readTimeOut) {
     ...
     * 全局写入超时时间
     */
    public SuperHttpManager setWriteTimeOut(long writeTimeout) {
     ...
     * 全局连接超时时间
     */
    public SuperHttpManager setConnectTimeout(long connectTimeout) {
     ...
     * 全局的缓存模式
     */
    public SuperHttpManager setCacheMode(CacheMode cacheMode) {
     ...
     * 全局的缓存大小,默认50M
     */
    public SuperHttpManager setCacheMaxSize(long maxSize) {
     ...
     */
    public SuperHttpManager setCacheDirectory(File directory) {
     ...
     * 全局设置缓存的转换器
     */
    public SuperHttpManager setCacheDiskConverter(IDiskConverter converter) {
     ...
     * 全局设置OkHttp的缓存,默认是3天
     */
    public SuperHttpManager setHttpCache(Cache cache) {
        this.mCache = cache;
        return this;
    }
     ...
     * 添加全局拦截器
     */
    public SuperHttpManager addInterceptor(Interceptor interceptor) {
     ...
     * 添加全局网络拦截器
     */
    public SuperHttpManager addNetworkInterceptor(Interceptor interceptor) {
     ...
     * 全局设置代理
     */
    public SuperHttpManager setOkproxy(Proxy proxy) {
     ...
     * 全局设置请求的连接池
     */
    public SuperHttpManager setOkconnectionPool(ConnectionPool connectionPool) {
     ...
     */
    public SuperHttpManager setOkclient(OkHttpClient client) {
     ...
     */
    public SuperHttpManager addConverterFactory(Converter.Factory factory) {
     ...
     */
    public SuperHttpManager addCallAdapterFactory(CallAdapter.Factory factory) {
     ...
     */
    public SuperHttpManager setCallbackExecutor(Executor executor) {
     ...
     */
    public SuperHttpManager setCallFactory(okhttp3.Call.Factory factory) {
     ...
     * 全局设置baseurl
     */
    public SuperHttpManager setBaseUrl(String baseUrl) {
