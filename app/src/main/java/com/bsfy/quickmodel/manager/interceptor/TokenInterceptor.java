package com.bsfy.quickmodel.manager.interceptor;
...
import com.bsfy.quickmodel.cache.login.LoginCache;
import com.bsfy.quickmodel.cache.login.LoginInfo;
import com.bsfy.quickmodel.manager.token.AuthModel;
import com.bsfy.quickmodel.manager.token.TokenManager;
import com.bsfy.quickmodel.utils.DateTimeUtils;
...
//                    case ComParamContact.Code.NO_ACCESS_TOKEN://缺少授权信息,没有accessToken,应该是没有登录
    ...
    public void refreshToken() throws IOException {
        SuperHttpManager.post(ComParamContact.Token.PATH)
                .params(ComParamContact.Common.REFRESH_TOKEN, TokenManager.getInstance().getAuthModel().getToken())
                .sign(false)
                .accessToken(false)
                .timeStamp(true)
    ...
    private Response processAccessTokenError(Interceptor.Chain chain, Request request) throws IOException {
        return processError(chain, request, ComParamContact.Common.ACCESSTOKEN, TokenManager.getInstance().getAuthModel().getToken());
    }
...
//        originalResponse.body().close();
    ...
    private String processSign(FormBody body, String path) {
        ...
        for (int i = 0; i < body.size(); i++) {
            ...
            if (name.equals(ComParamContact.Common.ACCESSTOKEN)) {
                value = TokenManager.getInstance().getAuthModel().getToken();
    ...
    private Response processRefreshTokenError(Interceptor.Chain chain, Request request) throws IOException {
        return processError(chain, request, ComParamContact.Common.REFRESH_TOKEN, TokenManager.getInstance().getAuthModel().getToken());
    }
    ...
    private void reLogin() throws IOException {
        ...
        SuperHttpManager.post(ComParamContact.Login.PATH)
